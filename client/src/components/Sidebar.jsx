...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  const {
    chats, setSelectedChat, theme, setTheme, 
    user, navigate, createNewChat, axios, setChats, 
    fetchUserChats, setToken, token
  } = useAppContext();
  ...
  return (
    <div className={`flex flex-col h-screen min-w-72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30
      ...
      <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 rounded-md'>
        ...
        <input onChange={(e) => setSearch(e.target.value)} type="text" value={search} placeholder='Search Conversations'
      ...
      <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
        {
          ...
          includes(search.toLowerCase())).map((chat) => (
            ...
            flex justify-between group'>
              ...
              <img onClick = {e => toast.promise(deleteChat(e, chat._id), {loading: 'deleting...'})} src={assets.bin_icon} className='hidden group-hover:block w-4
      ...
      dark:border-white/15 rounded-md cursor-pointer hover:scale-103 transition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p>Purchase credits to use Hub</p>
        </div>
      </div>
